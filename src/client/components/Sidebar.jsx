            ...
            (
                <aside class="w-70 bg-white shadow-lg p-4 space-y-6">
                    <div class="flex gap-3 items-center w-full bg-gray-100">
                        <img src={headshot || user?.photoUrl } alt="" class="rounded-full w-8 h-8 object-cover"/>
                        <h2 class="text-lg font-semibold">{user?.email || 'Abdullah'}</h2>
                    </div>
                ...
                <aside class="w-fit bg-white shadow-lg p-4">
                    <div class="relative flex flex-col flex-1 justify-between gap-10 ">
                        <img src={headshot || user?.photoUrl } alt="" class="rounded-full w-10 h-10 object-cover"/>
                        <button class="" onClick={() => setTabPaneOpen(!tabPaneOpen)}>
                            <HiMenu size={30} class=" left-60 top-6"/>
                        </button>
                        <button>
...
export const TopNav = ({ activeTab, setActiveTab }) => {
    ...
    return (
        ...
        <div class="flex min-w-auto border-b-white border-b-3 items-center bg-white">
            {tabs.map((tab) => {
                return (
                    <button onClick={() => setActiveTab(tab.id)} class={`flex gap-2 items-center w-full text-left p-4 rounded ${activeTab === tab.id ? "bg-blue-600 text-white" : "hover:bg-gray-200"}`}>
